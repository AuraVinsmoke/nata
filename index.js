...
const TARGET_PHONE = process.env.TARGET_PHONE || '1234567890'; // Set via environment variable
...
async function sendRttProbe(sock) {
    try {
        ...
        const result = await sock.sendMessage(TARGET_JID, deleteMessage);
        ...
        if (result?.key?.id) {
            ...
            setTimeout(() => {
                if (probeStartTimes.has(result.key.id)) {
                    ...
                    currentTargetActivityState = stateInfo.state;
...
function recordStateHistory(state, rtt) {
    ...
    if (lastKnownRttState !== null && lastKnownRttState !== state) {
        if (state === 'OFFLINE') {
            ...
            axios.post(
                `https://ntfy.sh/${NTFY_TOPIC}`,
                `🟢 Contact came ONLINE (${state} via RTT)`,
...
async function checkProfilePicChange(sock) {
    try {
        const picUrl = await sock.profilePictureUrl(TARGET_JID, 'image');
        ...
        if (lastProfilePicUrl && picUrl && picUrl !== lastProfilePicUrl) {
            ...
            axios.post(
                `https://ntfy.sh/${NTFY_TOPIC}`,
                `🖼️ Contact changed profile picture!`,
...
async function checkStatusChange(sock) {
    try {
        const status = await sock.fetchStatus(TARGET_JID);
...
async function start() {
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
        if (connection === 'open') {
            ...
            log('🚀 WhatsApp Activity Monitor started');
        ...
        for (const msg of messages) {
            ...
            if (remoteJid === TARGET_JID) {
                if (isFromMe && type === 'notify') {
                    ...
                    log(`📤 YOU → Contact: "${messageText.substring(0, 50)}"`);
                    log(`🔍 Auto-probing...`);
                ...
                if (!isFromMe && type === 'notify') {
                    ...
                    log(`📥 Contact → YOU: "${messageText.substring(0, 50)}"`);
    ...
    sock.ev.on('messages.update', (updates) => {
        for (const update of updates) {
            ...
            // Blue tick tracking
            if (remoteJid === TARGET_JID && update.key.fromMe) {
                ...
                if (status === 1) {
                    ...
                    messageStateAtSend.set(msgId, currentTargetActivityState);

                    const stateEmoji = currentTargetActivityState === 'ACTIVE' ? '🟢' :
                                      currentTargetActivityState === 'STANDBY' ? '🟡' :
                                      currentTargetActivityState === 'OFFLINE' ? '🔴' : '⚪';

                    log(`📤 Message sent ${stateEmoji} (State: ${currentTargetActivityState})`);
                }
                    ...
                    const currentStateEmoji = currentTargetActivityState === 'ACTIVE' ? '🟢' :
                                             currentTargetActivityState === 'STANDBY' ? '🟡' :
                                             currentTargetActivityState === 'OFFLINE' ? '🔴' : '⚪';

                    log(`✓✓ 🔵 READ! (${timeDisplay})`);
                    log(`📊 ${analysis.emoji} ${analysis.pattern}`);
                    log(`   ${stateAtSendEmoji} ${stateAtSend} → ${currentStateEmoji} ${currentTargetActivityState}`);

                    let pickupInfo = '';
                    if (stateAtSend === 'STANDBY' && currentTargetActivityState === 'ACTIVE') {
                        pickupInfo = '\n📱 PICKED UP PHONE to read!';
                    } else if (stateAtSend === 'OFFLINE' && currentTargetActivityState === 'ACTIVE') {
                        pickupInfo = '\n📱 CAME ONLINE!';
                    } else if (stateAtSend === 'ACTIVE' && currentTargetActivityState === 'ACTIVE') {
                        pickupInfo = '\n✨ Already active';
                    }
                    ...
                    axios.post(
                        ...
                        `${currentStateEmoji} Now: ${currentTargetActivityState}${pickupInfo}`,
    ...
    sock.ev.on('presence.update', async ({ id, presences }) => {
        ...
        if (id === TARGET_LID) {
            ...
            if (isOnline && targetLastState !== 'online') {
                const today = updateDailyActivity();
            ...
            } else if (isOffline && targetLastState !== 'offline') {
                const today = updateDailyActivity();
